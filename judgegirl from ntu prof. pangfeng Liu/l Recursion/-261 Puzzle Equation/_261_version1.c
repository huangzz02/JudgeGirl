...
void dfs(int pos, char *unique, int num_unique, int *val, char *str1, char *str2, char *str3, bool *digit_used) {
    if (pos == num_unique) {
    ...
    for (int d = 0; d <= 9; d++) {
        ...
        val[unique[pos] - 'A'] = d;
        digit_used[d] = true;
        dfs(pos + 1, unique, num_unique, val, str1, str2, str3, digit_used);
        digit_used[d] = false;
    }
}
...
void solve(char *str1, char *str2, char *str3) {
    ...
    */
    ...
    dfs(0, unique, num_unique, val, str1, str2, str3, digit_used);
}
...
/*
    This program should be the most complete version,
    but because the problem is simplified in difficulty,
    it allows different letters to use the same number,
    so this version may not be applicable.

    example input:
    AB CD 242
    expected output (in this question):
    22 x 11 = 242
    actual output:
    (no output)
*/